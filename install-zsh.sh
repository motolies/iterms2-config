...
usage() {
  printf '%s\n' \
    ...
    "" \
    "히스토리 자동완성(인라인 제안·부분 문자열 검색·퍼지 목록)과" \
    "마지막 작업 디렉터리 기억을 설치합니다." \
    "" \
    "옵션:" \
...
printf '%s\n' \
  ...
  "" \
  "마지막 작업 디렉터리 기억" \
  "  iTerm2를 완전히 종료했다가 다시 켰을 때, 홈에서 시작하는 첫 셸만 마지막 폴더로 이동합니다." \
  "  기록 위치: ${XDG_STATE_HOME:-$HOME/.local/state}/iterms2-config/last-dir" \
  "" \
